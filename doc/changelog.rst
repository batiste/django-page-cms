...
==============
    ...
    * Renamed python module from django_gerbi.to django_gerbi (see migration notes).
    * ...
...
==============
    ...
    * Tests are not executed when you execute ./manage.py test, unless explicity enabled with DJANGO_GERBI_ENABLE_TESTS.
...
==============
    ...
    * Fix a bug with `django_gerbi.dynamic_tree_menu` template tag and multiple roots in a django_gerbi.tree.
    ...
    * Add a DJANGO_GERBI_CONTENT_REVISION_DEPTH setting to limit the amount of revision we want to keep.
    * Fix a bug so the CMS can run without django-taggit installed.
    * Fix a bug with placeholder and template inheritance.
    * The `django_gerbi.root` URL doesn't need to be specified anymore. But you can still
...
==============
    ...
    * Don't raise a 404 when the LANGUAGE_CODE language is not present in the DJANGO_GERBI_LANGUAGES list.
...
=============
    ...
    * Fix bug with the default value of DJANGO_GERBI_TEMPLATES doesn't trigger an error in the admin
      when unspecified.
    * Add a delete image feature to the image placeholder.
    * Make root page url '/' work with the DJANGO_GERBI_USE_LANGUAGE_PREFIX option.
...
=============
    ...
    * New setting DJANGO_GERBI_HIDE_SITES to hide the sites. When True the CMS only
    ...
    * DEFAULT_DJANGO_GERBI_TEMPLATE is rennomed into PAGE_DEFAULT_TEMPLATE. This setting will still continue to work.
    * Add a new template tag get_page to insert page object into the context.

Release 1.1.2
=============

    * Change the default value of DJANGO_GERBI_TAGGING and PAGE_TINYMCE to `False`
...
=============
    ...
    * DJANGO_GERBI_TEMPLATES setting can also be a callable.
    * DJANGO_GERBI_UPLOAD_ROOT setting enable you to choose where files are uploaded.
    ...
    * The context variable `django_gerbi. has been renamed to `django_gerbi.naviagtion` to avoid
...
to enable the delegation of the django_gerbi.rendering to a 3rd party application::

    ALTER TABLE django_gerbi.page ADD COLUMN delegate_to varchar(100) NULL;
