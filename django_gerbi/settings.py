# -*- coding: utf-8 -*-
"""Convenience module that provides default settings for the ``django_gerbi``
...
DJANGO_GERBI_DEFAULT_TEMPLATE = get_setting('DJANGO_GERBI_DEFAULT_TEMPLATE',
    'DEFAULT_DJANGO_GERBI_TEMPLATE', raise_error=True)
...
DJANGO_GERBI_TEMPLATES = get_setting('DJANGO_GERBI_TEMPLATES',
    default_value=())


def get_page_templates():
    """The callable that is used by the CMS."""
    if callable(DJANGO_GERBI_TEMPLATES):
        return DJANGO_GERBI_TEMPLATES()
    else:
        return DJANGO_GERBI_TEMPLATES

# Set ``DJANGO_GERBI_TAGGING`` to ``False`` if you do not wish to use the
# ``django-taggit application.
DJANGO_GERBI_TAGGING = getattr(settings, 'DJANGO_GERBI_TAGGING', False)
if DJANGO_GERBI_TAGGING and "taggit" not in getattr(settings, 'INSTALLED_APPS', []):
    raise ImproperlyConfigured('django-taggit could not be found.\n'
                               ...
                               'setting DJANGO_GERBI_TAGGING to False.')

# Set this to ``True`` if you wish to use the ``django-tinymce`` application.
DJANGO_GERBI_TINYMCE = getattr(settings, 'DJANGO_GERBI_TINYMCE', False)
if DJANGO_GERBI_TINYMCE and "tinymce" not in getattr(settings, 'INSTALLED_APPS', []):
    raise ImproperlyConfigured('django-tinymce could not be found.\n'
                               'Please make sure you\'ve installed it '
                               'correctly or disable the tinymce feature by '
                               'setting DJANGO_GERBI_TINYMCE to False.')

# Set ``DJANGO_GERBI_UNIQUE_SLUG_REQUIRED`` to ``True`` to enforce unique slug names
# for all pages.
DJANGO_GERBI_UNIQUE_SLUG_REQUIRED = getattr(settings, 'DJANGO_GERBI_UNIQUE_SLUG_REQUIRED',
                                    False)

# Set ``DJANGO_GERBI_CONTENT_REVISION`` to ``False`` to disable the recording of
# pages revision information in the database
DJANGO_GERBI_CONTENT_REVISION = getattr(settings, 'DJANGO_GERBI_CONTENT_REVISION', True)

# Define the number of revisions too keep in the database. Set to None
# if you want to keep everything
DJANGO_GERBI_CONTENT_REVISION_DEPTH = getattr(settings,
    'DJANGO_GERBI_CONTENT_REVISION_DEPTH', 10)
...
DJANGO_GERBI_LANGUAGES = get_setting('DJANGO_GERBI_LANGUAGES', raise_error=True)
...
DJANGO_GERBI_DEFAULT_LANGUAGE = get_setting('DJANGO_GERBI_DEFAULT_LANGUAGE',
...
for lang in DJANGO_GERBI_LANGUAGES:
    extra.append(
        ('can_manage_' + lang[0].replace('-', '_'),
        'Manage' + ' ' + lang[1])
    )

DJANGO_GERBI_EXTRA_PERMISSIONS = getattr(settings, 'DJANGO_GERBI_EXTRA_PERMISSIONS', extra)

# DJANGO_GERBI_LANGUAGE_MAPPING should be assigned a function that takes a single
...
DJANGO_GERBI_LANGUAGE_MAPPING = getattr(settings, 'DJANGO_GERBI_LANGUAGE_MAPPING', lambda l: l)
...
# framework
DJANGO_GERBI_USE_SITE_ID = getattr(settings, 'DJANGO_GERBI_USE_SITE_ID', False)

# Set DJANGO_GERBI_HIDE_SITES to make the sites that appear uneditable and only allow
# editing and creating of pages on the current site
DJANGO_GERBI_HIDE_SITES = getattr(settings, 'DJANGO_GERBI_HIDE_SITES', False)
...
DJANGO_GERBI_USE_LANGUAGE_PREFIX = getattr(settings, 'DJANGO_GERBI_USE_LANGUAGE_PREFIX',
                                                                        False)

# Set this to True to raise an error 404 if the used URL path is
# not strictly the same than the page.
DJANGO_GERBI_USE_STRICT_URL = getattr(settings, 'DJANGO_GERBI_USE_STRICT_URL', False)

# Assign a list of placeholders to DJANGO_GERBI_CONTENT_REVISION_EXCLUDE_LIST
# to exclude them from the revision process.
DJANGO_GERBI_CONTENT_REVISION_EXCLUDE_LIST = getattr(settings,
    'DJANGO_GERBI_CONTENT_REVISION_EXCLUDE_LIST', ()
)

# Set ``DJANGO_GERBI_SANITIZE_USER_INPUT`` to ``True`` to sanitize the user input with
# ``html5lib``
DJANGO_GERBI_SANITIZE_USER_INPUT = getattr(settings, 'DJANGO_GERBI_SANITIZE_USER_INPUT', False)

# URL that handles django_gerbi media and uses <STATIC_URL>/django_gerbi by default.
DJANGO_GERBI_MEDIA_URL = get_setting('DJANGO_GERBI_MEDIA_URL')
if not DJANGO_GERBI_MEDIA_URL:
    media_url = get_setting('STATIC_URL', 'MEDIA_URL', raise_error=True)
    DJANGO_GERBI_MEDIA_URL = str(media_url) + 'django_gerbi/'

# Hide the slug's of the first root page ie: ``/home/`` becomes ``/``
DJANGO_GERBI_HIDE_ROOT_SLUG = getattr(settings, 'DJANGO_GERBI_HIDE_ROOT_SLUG', False)
...
DJANGO_GERBI_SHOW_START_DATE = getattr(settings, 'DJANGO_GERBI_SHOW_START_DATE', False)
...
DJANGO_GERBI_SHOW_END_DATE = getattr(settings, 'DJANGO_GERBI_SHOW_END_DATE', False)
...
# ]
#
DJANGO_GERBI_CONNECTED_MODELS = getattr(settings, 'DJANGO_GERBI_CONNECTED_MODELS', False)
...
DJANGO_GERBI_LINK_FILTER = getattr(settings, 'DJANGO_GERBI_LINK_FILTER', False)
...
# is called by subclassing ``django_gerbi.views.Details``.
DJANGO_GERBI_EXTRA_CONTEXT = getattr(settings, 'DJANGO_GERBI_EXTRA_CONTEXT', None)

# This setting is the name of a sub-folder where uploaded content, like
# placeholder images, is placed.
DJANGO_GERBI_UPLOAD_ROOT = getattr(settings, 'DJANGO_GERBI_UPLOAD_ROOT', 'upload')

# Disable the tests by default so they don't execute when the user
# execute manage.py test
DJANGO_GERBI_ENABLE_TESTS = getattr(settings, 'DJANGO_GERBI_ENABLE_TESTS', False)

# Enable use of legacy template tag names (prefixed by ``pages_``) as well as new
# ``gerbi_*`̀` template tags names.

DJANGO_GERBI_LEGACY_TAG_NAMES = getattr( settings, 'DJANGO_GERBI_LEGACY_TAG_NAMES',
					 True )
