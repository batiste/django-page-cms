...
from django_gerbi.http import auto_render, get_language_from_request
...
def list_pages_ajax(request, invalid_move=False):
    ...
    return "admin/django_gerbi/page/change_list_table.html", context
...
def delete_content(request, page_id, language_id):
    ...
    destination = request.REQUEST.get('next', request.META.get('HTTP_REFERER',
        '/admin/django_gerbi/page/%s/' % page_id))
...
def traduction(request, page_id, language_id):
    ...
    return 'django_gerbi/traduction_helper.html', {
        'page':page,
        'lang':lang,
