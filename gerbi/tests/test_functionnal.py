...
from gerbi.models import Page, Content, PageAlias
...
class FunctionnalTestCase(TestCase):
    ...
    def test_details_view(self):
        ...
        page_data['template'] = 'gerbi/examples/index.html'
        response = c.post('/admin/gerbi/page/add/', page_data)
        self.assertRedirects(response, '/admin/gerbi/page/')
    ...
    def test_edit_page(self):
        ...
        response = c.post('/admin/gerbi/page/%d/' % page.id, page_data)
        self.assertRedirects(response, '/admin/gerbi/page/')
    ...
    def test_site_framework(self):
        ...
        from gerbi import settings as pages_settings
    ...
    def test_languages(self):
        ...
        if major >= 1 and middle > 0:
            response = c.get('/admin/gerbi/page/%d/?language=de' % page.id)
            self.assertContains(response, 'value="de"')
    ...
    def test_placeholder(self):
        ...
        c.login(username='batiste', password='b')
        page_data = self.get_new_page_data()
        page_data['template'] = 'gerbi/examples/nice.html'
        response = c.post('/admin/gerbi/page/add/', page_data)
        page = Page.objects.all()[0]
        response = c.get('/admin/gerbi/page/%d/' % page.id)
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_admin_view(self):
        ...
        response = c.post('/admin/gerbi/page/%d/change-status/' %
            page.id, {'status':Page.DRAFT})
        page = Content.objects.get_content_slug_by_slug('page-1').page
        self.assertEqual(page.status, Page.DRAFT)

        url = '/admin/gerbi/page/%d/modify-content/title/en-us/' % page.id
        response = c.post(url, {'content': 'test content'})
        self.assertEqual(page.title(), 'test content')

        # TODO: realy test these methods
        url = '/admin/gerbi/page/%d/traduction/en-us/' % page.id
        response = c.get(url)
        self.assertEqual(response.status_code, 200)

        url = '/admin/gerbi/page/%d/sub-menu/' % page.id
        response = c.get(url)
        self.assertEqual(response.status_code, 200)

        url = '/admin/gerbi/page/%d/get-content/%d/' % (page.id,
            Content.objects.get_content_slug_by_slug('page-1').id)

        response = c.get(url)
        self.assertEqual(response.status_code, 200)

        url = '/admin/gerbi/page/%d/delete-content/en-us/' % page.id
        response = c.get(url)
    ...
    def test_page_alias(self):
        ...
        # now check whether we can retrieve the gerbi.
    ...
    def test_ajax_language(self):
        ...
        response = c.get('/admin/gerbi/page/')
        self.assertEqual(response.status_code, 200)
        ...
        request.
        """
        response = c.post('/admin/gerbi/page/%d/move-page/' % child_1.id,
    ...
    def test_tree_admin_interface(self):
        ...
        response = c.post('/admin/gerbi/page/%d/move-page/' % child_2.id,
        ...
        page_data['slug'] = 'child-1'
        response = c.post('/admin/gerbi/page/%d/' % child_2.id, page_data)
        self.assertEqual(response.status_code, 200)
        setattr(pages_settings, "GERBI_UNIQUE_SLUG_REQUIRED", True)
        response = c.post('/admin/gerbi/page/%d/' % child_2.id, page_data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_untranslated(self):
        ...
        page_data['template'] = 'gerbi/tests/untranslated.html'
        response = c.post('/admin/gerbi/page/add/', page_data)
        self.assertRedirects(response, '/admin/gerbi/page/')
        ...
        page_data['untrans'] = ''
        response = c.get('/admin/gerbi/page/%d/?language=fr-ch' % page.id)
