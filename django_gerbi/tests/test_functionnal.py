...
class FunctionnalTestCase(TestCase):
    ...
    def test_add_page(self):
        ...
        response = c.get('/admin/django_gerbi/page/add/')
    ...
    def test_details_view(self):
        ...
        page_data['template'] = 'django_gerbi/examples/index.html'
        response = c.post('/admin/django_gerbi/page/add/', page_data)
        self.assertRedirects(response, '/admin/django_gerbi/page/')
    ...
    def test_edit_page(self):
        ...
        response = c.get('/admin/django_gerbi/page/%d/' % page.id)
        self.assertEqual(response.status_code, 200)
        page_data['title'] = 'changed title'
        page_data['body'] = 'changed body'
        response = c.post('/admin/django_gerbi/page/%d/' % page.id, page_data)
        self.assertRedirects(response, '/admin/django_gerbi/page/')
    ...
    def test_languages(self):
        ...
        if major >= 1 and middle > 0:
            response = c.get('/admin/django_gerbi/page/%d/?language=de' % page.id)
            self.assertContains(response, 'value="de"')
    ...
    def test_placeholder(self):
        ...
        c.login(username='batiste', password='b')
        page_data = self.get_new_page_data()
        page_data['template'] = 'django_gerbi/examples/nice.html'
        response = c.post('/admin/django_gerbi/page/add/', page_data)
        page = Page.objects.all()[0]
        response = c.get('/admin/django_gerbi/page/%d/' % page.id)
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_admin_view(self):
        ...
        response = c.post('/admin/django_gerbi/page/%d/change-status/' %
            page.id, {'status':Page.DRAFT})
        page = Content.objects.get_content_slug_by_slug('page-1').page
        self.assertEqual(page.status, Page.DRAFT)

        url = '/admin/django_gerbi/page/%d/modify-content/title/en-us/' % page.id
        response = c.post(url, {'content': 'test content'})
        self.assertEqual(page.title(), 'test content')

        # TODO: realy test these methods
        url = '/admin/django_gerbi/page/%d/traduction/en-us/' % page.id
        response = c.get(url)
        self.assertEqual(response.status_code, 200)

        url = '/admin/django_gerbi/page/%d/sub-menu/' % page.id
        response = c.get(url)
        self.assertEqual(response.status_code, 200)

        url = '/admin/django_gerbi/page/%d/get-content/slug/' % page.id
        response = c.get(url)
        self.assertEqual(response.status_code, 200)

        url = '/admin/django_gerbi/page/%d/delete-content/en-us/' % page.id
        response = c.get(url)
    ...
    def test_page_alias(self):
        ...
        # now check whether we can retrieve the django_gerbi.
    ...
    def test_ajax_language(self):
        ...
        response = c.get('/admin/django_gerbi/page/')
        self.assertEqual(response.status_code, 200)
        self.assertTrue('Auteur' in response.content)

        # Create some django_gerbi (taken from test_tree_admin_interface)
        page_data = self.get_new_page_data()
        page_data['slug'] = 'root'
        response = c.post('/admin/django_gerbi/page/add/', page_data)
        ...
        request.
        """
        response = c.post('/admin/django_gerbi/page/%d/move-page/' % child_1.id,
    ...
    def test_view_context(self):
        ...
        from django_gerbi.utils import get_request_mock
    ...
    def test_tree_admin_interface(self):
        ...
        response = c.post('/admin/django_gerbi/page/%d/move-page/' % child_2.id,
    ...
    def test_untranslated(self):
        ...
        page_data['template'] = 'django_gerbi/tests/untranslated.html'
        response = c.post('/admin/django_gerbi/page/add/', page_data)
        self.assertRedirects(response, '/admin/django_gerbi/page/')
        ...
        page_data['untrans'] = ''
        response = c.get('/admin/django_gerbi/page/%d/?language=fr-ch' % page.id)
