...
from gerbi.http import auto_render, AutoRenderHttpError, get_request_mock
...
class AutoRenderTestCase(TestCase):
    ...
    def test_auto_render(self):
        ...
        self.assertEqual(response.content,
                         "template_name: 'gerbi/tests/auto_render.txt', "
                         "only_context: ''\n")
        response = testview(request_mock, template_name='gerbi/tests/auto_render2.txt')
        self.assertEqual(response.__class__, HttpResponse)
        self.assertEqual(response.content,
                         "alternate template_name: 'gerbi/tests/auto_render2.txt', "
                         "only_context: ''\n")
    ...
    def test_auto_render_httpresponse(self):
        ...
        self.assertEqual(testview(None, only_context=False).__class__,
                         HttpResponse)
        response = testview(None, template_name='gerbi/tests/auto_render2.txt')
        self.assertEqual(response.__class__, HttpResponse)
        self.assertEqual(response.content,
                         "[('args', ()), ('kwargs', {}), ('request', None)]")
