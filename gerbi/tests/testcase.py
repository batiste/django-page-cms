...
from gerbi.testproj import test_settings
...
class TestCase(TestCase):
    ...
    def setUp(self):
        ...
        setattr(settings, 'ROOT_URLCONF', 'gerbi.testproj.urls')
        clear_url_caches()
    ...
    def get_page_url(self, path=''):
        return reverse('django-gerbi-details-by-path', args=[path])
    ...
    def reset_urlconf(self):
        ...
        reload(import_module('gerbi.urls'))
        reload(import_module('gerbi.testproj.urls'))
        clear_url_caches()
    ...
    def new_page(self, content={'title': 'test-page'}, language='en-us'):
        ...
        page = Page.objects.create(author=author, status=Page.PUBLISHED,
            template='gerbi/examples/index.html')
    ...
    def create_new_page(self, client=None, draft=False):
        ...
        response = client.post('/admin/gerbi/page/add/', page_data)
