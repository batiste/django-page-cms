...
class PagePermission(authority.permissions.BasePermission):
    """Handle the :class:`Page <gerbi.models.Page>` permissions."""
    ...
    def check(self, action, page=None, lang=None, method=None):
        ...
        if action == 'add':
            ...
            return True
        if action == 'freeze':
            perm = self.user.has_perm('gerbi.can_freeze')
            if perm:
                return True
            return False
        if action == 'publish':
            perm = self.user.has_perm('gerbi.can_publish')
            if perm:
                return True
            return False
    ...
    def has_change_permission(self, page, lang, method=None):
        ...
        if lang:
            ...
            perm = self.user.has_perm(
                'gerbi.can_manage_%s' % lang.replace('-', '_')
            )
            if perm:
                return True
